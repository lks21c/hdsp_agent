...
**리팩토링 요청 시 반드시 "최근 셀 내용"에서 다음을 찾으세요:**

1. **데이터 로드 셀**: `read_csv`, `read_excel`, `read_parquet`, `read_json` 등이 포함된 셀
   - 이 셀에서 `df`, `data`, `dataset` 등 주요 변수가 정의됨
   - ⚠️ 이 셀이 변환 대상이면 **반드시 가장 먼저 수정!**

2. **라이브러리 import 셀**: `import dask.dataframe as dd` 등
   - pandas↔dask 변환 시 import문도 함께 수정 필요

3. **변수 정의 체인**: 셀 A에서 df 로드 → 셀 B에서 df 가공 → 셀 C에서 df 사용
   - A → B → C 순서로 수정해야 함 (정의 먼저, 사용 나중!)

**예시: "dask를 pandas로 바꿔줘"**
```
최근 셀 내용을 분석:
- 셀 0: import dask.dataframe as dd     ← import 수정 필요!
- 셀 2: df = dd.read_csv('data.csv')    ← df 정의! 반드시 먼저 수정!
- 셀 4: df.describe().compute()          ← df 사용, 나중에 수정
- 셀 6: df.groupby('A').mean().compute() ← df 사용, 마지막 수정

올바른 수정 순서: 셀 0 → 셀 2 → 셀 4 → 셀 6
```

**🚨 절대 하면 안 되는 실수:**
- df를 로드하는 셀(셀 2)을 건너뛰고 df를 사용하는 셀(셀 4, 6)만 수정
- → df가 여전히 dask DataFrame이거나 정의되지 않아 **NameError 발생!**

### 🔍 리팩토링 요청 처리 체크리스트:
0. [ ] **⭐ 핵심 변수(df 등)를 정의하는 셀 먼저 식별** (read_csv, read_excel 등)
1. [ ] 아래 "최근 셀 내용"에서 변경 대상 코드 포함된 셀 인덱스 모두 확인
2. [ ] 변수 정의 셀 → 변수 사용 셀 순서로 MODIFY 계획 수립
3. [ ] **⚠️ 셀 인덱스 순서대로 수정** (0, 1, 2, 3 순서로!)
4. [ ] CREATE는 기존 코드와 무관한 새 분석에만 사용
5. [ ] 동일한 기능을 하는 새 셀을 만들지 않음

### 🔴 실행 순서 규칙 (CRITICAL!):
**MODIFY 작업은 반드시 셀 인덱스가 작은 것부터 큰 순서로 실행해야 합니다!**

이유: 셀 N을 실행하면, 셀 0~(N-1)이 이미 실행되어 변수가 정의되어 있어야 합니다.
- 잘못된 순서: 셀 4 수정 → 셀 2 수정 (셀 4에서 df 사용 시 에러!)
- 올바른 순서: 셀 2 수정 (df 로드) → 셀 4 수정 (df 사용)

**예시: "dask를 pandas로 바꿔줘" 요청**
...
- 최근 셀 내용 (⚠️ 패턴 표시 확인!):
  - 🔄 DASK 데이터 로드: **dask→pandas 변환 시 반드시 이 셀 먼저 수정!**
  - ⚠️ df 로드: **데이터 정의 셀, 먼저 수정 필요**
  - 🔄 DASK import: **변환 시 수정 필요**
{recent_cells}
...
def format_plan_prompt(
    ...
    # 데이터 로드 패턴 감지 함수
    def detect_data_load_patterns(source: str) -> list:
        """셀 소스에서 데이터 로드 패턴 감지"""
        patterns = []
        # DataFrame 로드 패턴
        if re.search(r'\.read_csv\s*\(', source):
            patterns.append("⚠️ df 로드 (read_csv)")
        if re.search(r'\.read_excel\s*\(', source):
            patterns.append("⚠️ df 로드 (read_excel)")
        if re.search(r'\.read_parquet\s*\(', source):
            patterns.append("⚠️ df 로드 (read_parquet)")
        if re.search(r'\.read_json\s*\(', source):
            patterns.append("⚠️ df 로드 (read_json)")
        # Dask 특정 패턴
        if re.search(r'dd\.read_', source) or re.search(r'dask\.dataframe\.read_', source):
            patterns.append("🔄 DASK 데이터 로드 - 변환 시 반드시 이 셀 먼저 수정!")
        # import 패턴
        if re.search(r'import\s+dask', source):
            patterns.append("🔄 DASK import - 변환 시 수정 필요")
        if re.search(r'import\s+pandas', source) or re.search(r'from\s+pandas', source):
            patterns.append("📦 pandas import")
        return patterns
    ...
    for i, cell in enumerate(recent_cells):
        ...
        # 데이터 로드 패턴 감지
        load_patterns = detect_data_load_patterns(source) if cell_type == 'code' else []
        pattern_text = " | ".join(load_patterns) if load_patterns else ""

        if pattern_text:
            recent_cells_text += f"\n[셀 {cell_index}] ({cell_type}) {pattern_text}:\n```\n{source}\n```\n"
        else:
            recent_cells_text += f"\n[셀 {cell_index}] ({cell_type}):\n```\n{source}\n```\n"
