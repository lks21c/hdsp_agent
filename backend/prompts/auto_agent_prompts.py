...
**기존 노트북을 수정하는 요청인 경우**, 다음 규칙을 반드시 따르세요:

1. **기존 셀 수정 (MODIFY)**: 기존 셀 내용을 바꿀 때
   - `"cellIndex": N, "operation": "MODIFY"` 사용
   - 예: 셀 2번의 코드를 개선 → {{"code": "개선된 코드", "cellIndex": 2, "operation": "MODIFY"}}
...
3. **끝에 추가 (CREATE)**: 노트북 맨 끝에 새 셀 추가 (기본 동작)
   - 파라미터 생략 시 기본값
   - 예: 새 분석 추가 → {{"code": "새 코드"}}

**판단 기준**:
- "셀 N을 수정해줘", "N번 셀 개선" → MODIFY
- "N번 셀 뒤에 추가해줘" → INSERT_AFTER
- 새로운 분석/시각화 추가 → CREATE (끝에 추가)
