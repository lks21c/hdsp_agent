...
class ReflectionEngine:
    ...
    def evaluate_checkpoint(
        ...
        if validation_criteria:
            for criterion in validation_criteria:
                # 간단한 키워드 매칭
                if any(word in execution_output.lower() for word in criterion.lower().split()):
                    criteria_passed += 1
    ...
    def analyze_execution(
        ...
        if execution_status == 'ok':
            ...
            # 출력 분석
            if execution_output:
                if 'error' in execution_output.lower() or 'warning' in execution_output.lower():
    ...
    def assess_impact(
        ...
        if not evaluation.checkpoint_passed:
            # 모든 후속 단계가 영향받을 가능성
            affected_steps = [step.get('stepNumber', i + 1) for i, step in enumerate(remaining_steps)]

            if analysis.failure_factors:
                # 심각한 오류 유형 확인
                critical_errors = ['정의되지 않은 변수', '필요한 모듈 import 누락', '문법 오류']
                if any(err in factor for factor in analysis.failure_factors for err in critical_errors):
            ...
            # 예상치 못한 결과가 있는 경우
            severity = ImpactSeverity.MINOR
            affected_steps = [remaining_steps[0].get('stepNumber', 1)] if remaining_steps else []
