...
class TestPlanGenerationUnit:
  ...
  "plan": {
    ...
    def plan_request_payload(self):
        ...
        return {
            ...
            "llmConfig": {
                "provider": "gemini",
                "gemini": {"apiKey": "test-api-key", "model": "gemini-2.5-flash"},
            },
        }
    ...
    def test_plan_request_empty_request(self, client):
        ...
        response = client.post(
            "/agent/plan", json={"request": "", "notebookContext": {"cellCount": 0}}
        )
    ...
    def test_plan_generation_success(
        self, mock_llm, client, mock_llm_plan_response, plan_request_payload
    ):
  ...
  "plan": {
    ...
    @patch("agent_server.routers.agent._call_llm")
    def test_plan_tool_calls_structure(
        self, mock_llm, client, mock_llm_plan_response, plan_request_payload
    ):
    ...
    @patch("agent_server.routers.agent._call_llm")
    def test_plan_final_answer_exists(
        self, mock_llm, client, mock_llm_plan_response, plan_request_payload
    ):
    ...
    def test_plan_generation_titanic_dask_eda(self, client, real_api_key):
        ...
        response = client.post(
            ...
            json={
                ...
                "llmConfig": {
                    "provider": "gemini",
                    "gemini": {"apiKey": real_api_key, "model": "gemini-2.5-flash"},
                },
            },
        )
      ...
      {"stepNumber": 3, "description": "Step 3", "toolCalls": [{"tool": "final_answer", "parameters": {"answer": "done"}}], "dependencies": [1, 2]}
        ...
        response = client.post(
            ...
            json={
                ...
                "llmConfig": {
                    "provider": "gemini",
                    "gemini": {"apiKey": "test", "model": "gemini-2.5-flash"},
                },
            },
        )
        ...
        for step in steps:
            for dep in step.get("dependencies", []):
                ...
                assert (
                    dep < step["stepNumber"]
                ), f"Dependency {dep} should be before step {step['stepNumber']}"
...
class TestLLMConfig:
    ...
    def test_plan_without_llm_config(self, client):
        ...
        response = client.post(
            "/agent/plan",
            json={"request": "테스트 요청", "notebookContext": {"cellCount": 0}},
        )
  ...
  "plan": {"totalSteps": 1, "steps": [{"stepNumber": 1, "description": "완료", "toolCalls": [{"tool": "final_answer", "parameters": {"answer": "done"}}], "dependencies": []}]}
        ...
        for llm_config in providers:
            ...
            assert (
                response.status_code == 200
            ), f"Failed for provider: {llm_config['provider']}"
