# HDSP Agent - Plan-and-Execute 아키텍처 흐름

## 개요

HDSP Agent는 HuggingFace Jupyter Agent에서 영감을 받은 **Plan-and-Execute 패턴**을 구현합니다.
사용자 요청을 받아 계획을 수립하고, 단계별로 실행하며, 오류 발생 시 적응적으로 재계획합니다.

---

## 전체 흐름도

```
┌─────────────────────────────────────────────────────────────────────────┐
│                         사용자 요청 (User Request)                        │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  1. Knowledge Base 동적 로딩 (Mini RAG)                                  │
│     - 요청에서 라이브러리 키워드 감지 (dask, polars, pyspark 등)            │
│     - 해당 라이브러리의 API 가이드 자동 로드                                │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  2. Planning (계획 수립)                                                  │
│     - System Prompt + 사용자 요청 + 노트북 컨텍스트 + 라이브러리 지식        │
│     - LLM이 실행 계획(steps) 생성                                         │
│     - 각 step은 tool 호출 정의 포함                                       │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  3. Pre-Validation (사전 검증)                                           │
│     - Ruff 기반 고속 정적 분석 (700+ 규칙, 보안/스타일/버그 패턴)            │
│     - Pyflakes fallback (Ruff 미설치 환경 대비)                           │
│     - AST 파싱으로 구문 분석 및 의존성 추출                                 │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  4. Step-by-Step Execution (단계별 실행)                                  │
│     - jupyter_cell: 셀 생성 및 실행                                       │
│     - markdown: 마크다운 셀 생성                                          │
│     - final_answer: 최종 답변 제공                                        │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
                    ┌───────────────┴───────────────┐
                    │                               │
                    ▼                               ▼
            ┌──────────────┐                ┌──────────────┐
            │   성공 (✓)    │                │   오류 (✗)    │
            └──────────────┘                └──────────────┘
                    │                               │
                    ▼                               ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  5. Reflection (실행 결과 분석)                                           │
│     - 체크포인트 평가 (부분 성공/완전 성공)                                 │
│     - 신뢰도 점수 계산                                                    │
│     - 다음 행동 권고: continue / adjust / retry / replan                  │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
                    ┌───────────────┴───────────────┐
                    │                               │
                    ▼                               ▼
            ┌──────────────┐                ┌──────────────┐
            │  계속 진행    │                │   재계획 필요  │
            └──────────────┘                └──────────────┘
                    │                               │
                    ▼                               ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  6. Adaptive Replanning (적응적 재계획)                                   │
│     - refine: 현재 step 수정                                             │
│     - insert_steps: 새로운 step 삽입                                     │
│     - replace_step: step 교체                                            │
│     - replan_remaining: 나머지 전체 재계획                                 │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
                            ┌──────────────┐
                            │   완료/실패   │
                            └──────────────┘
```

---

## 상세 단계 설명

### 1. Knowledge Base 동적 로딩 (Mini RAG)

사용자 요청에서 특정 라이브러리 키워드를 감지하면, 해당 라이브러리의 API 가이드를 자동으로 로드합니다.

**지원 라이브러리:**
| 트리거 키워드 | 로드되는 가이드 |
|--------------|----------------|
| `dask`, `dask.dataframe`, `dd.read` | `dask.md` |
| `polars`, `pl.read` | `polars.md` |
| `pyspark`, `spark` | `pyspark.md` |
| `vaex` | `vaex.md` |
| `modin` | `modin.md` |
| `ray` | `ray.md` |

**예시: Dask 요청 처리**
```
사용자: "dask로 대용량 CSV 파일을 병렬 처리해줘"
         ↓
1. "dask" 키워드 감지
2. backend/knowledge/libraries/dask.md 로드
3. 플래닝 프롬프트에 Dask API 가이드 포함
4. LLM이 올바른 Dask 문법으로 계획 생성
```

**코드 위치:** `backend/knowledge/loader.py`

---

### 2. Planning (계획 수립)

LLM에 전달되는 프롬프트 구성:

```
┌─────────────────────────────────────────────┐
│  System Prompt (역할 및 도구 정의)            │
├─────────────────────────────────────────────┤
│  📚 라이브러리 API 참조 (자동 로드됨)          │
│     - Mini RAG에서 로드된 가이드              │
├─────────────────────────────────────────────┤
│  노트북 컨텍스트                              │
│     - 현재 셀 코드                            │
│     - 실행 결과/오류                          │
│     - 임포트된 라이브러리 목록                 │
│     - 정의된 변수/함수                        │
├─────────────────────────────────────────────┤
│  사용자 요청                                  │
└─────────────────────────────────────────────┘
```

**출력 형식:**
```json
{
  "steps": [
    {
      "step_number": 1,
      "description": "데이터 로드",
      "tool": "jupyter_cell",
      "tool_input": "import dask.dataframe as dd\ndf = dd.read_csv('data.csv')"
    },
    {
      "step_number": 2,
      "description": "결과 설명",
      "tool": "final_answer",
      "tool_input": "데이터가 성공적으로 로드되었습니다."
    }
  ]
}
```

**코드 위치:** `backend/prompts/auto_agent_prompts.py` → `format_plan_prompt()`

---

### 3. Pre-Validation (사전 검증)

실행 전 코드 품질 검사를 수행합니다. **Ruff**를 기본 린터로 사용하고, **Pyflakes/AST**는 보조 도구로 활용합니다.

#### 검증 도구 비교

| 도구 | 특징 | 검사 범위 |
|------|------|----------|
| **Ruff** | Rust 기반 초고속 린터 (700+ 규칙) | F (Pyflakes), E/W (스타일), S (보안), B (버그 패턴), C90 (복잡도) |
| **Pyflakes** | 경량 린터 (Ruff fallback) | 미정의/미사용 변수, import 오류 |
| **AST** | Python 내장 파서 | 구문 분석, 의존성 추출 |

#### Ruff 규칙 카테고리

| 규칙 코드 | 카테고리 | 설명 | 심각도 |
|----------|---------|------|--------|
| F821 | undefined_name | 미정의 변수/함수 | ERROR |
| F401 | unused_import | 미사용 import | WARNING |
| F841 | unused_variable | 미사용 변수 | INFO |
| S102 | security | `exec()` 사용 감지 | WARNING |
| S105-S107 | security | 하드코딩된 비밀번호 | WARNING |
| B | best_practice | 버그 패턴 (flake8-bugbear) | WARNING |
| C901 | complexity | 함수 복잡도 초과 | WARNING |
| E9xx | syntax | 런타임 에러 (SyntaxError 등) | ERROR |

#### 검증 결과 예시:
```json
{
  "is_valid": false,
  "issues": [
    {"line": 3, "message": "[F821] Undefined name `unknown_var`", "severity": "error", "category": "undefined_name"},
    {"line": 1, "message": "[F401] `pandas` imported but unused", "severity": "warning", "category": "unused_import"},
    {"line": 5, "message": "[S102] Use of `exec` detected", "severity": "warning", "category": "security"}
  ],
  "summary": "검증 실패: 1개 오류, 2개 경고"
}
```

#### 노트북 특화 설정

Ruff 실행 시 노트북 환경에 맞게 일부 규칙을 무시합니다:
- `E501`: 라인 길이 제한 (노트북 셀은 긴 코드가 일반적)
- `W292`: 파일 끝 개행 없음 (셀 단위 실행이므로 불필요)

**코드 위치:** `backend/services/code_validator.py`

---

### 4. Step-by-Step Execution (단계별 실행)

**사용 가능한 도구:**

| 도구 | 설명 | 예시 |
|------|------|------|
| `jupyter_cell` | Python 코드 셀 생성 및 실행 | 데이터 처리, 시각화 |
| `markdown` | 마크다운 셀 생성 | 설명, 제목, 문서화 |
| `final_answer` | 최종 응답 제공 | 작업 완료 메시지 |

**실행 흐름:**
```
Step 1 실행 → 결과 확인 → Step 2 실행 → ... → 완료
      ↓              ↓
   오류 발생    →  Reflection으로 분기
```

**코드 위치:** `frontend/services/AgentOrchestrator.ts` → `executeStepWithRetry()`

---

### 5. Reflection (실행 결과 분석)

실행 결과를 분석하여 다음 행동을 결정합니다.

**분석 항목:**
- 체크포인트 달성 여부
- 오류 패턴 분석
- 신뢰도 점수 계산

**권고 행동:**
| 행동 | 조건 | 설명 |
|------|------|------|
| `continue` | 성공 | 다음 step 진행 |
| `adjust` | 경미한 오류 | 현재 step 수정 후 재시도 |
| `retry` | 일시적 오류 | 동일 step 재시도 |
| `replan` | 심각한 오류 | 전체 계획 재수립 |

**코드 위치:** `backend/services/reflection_engine.py`

---

### 6. Adaptive Replanning (적응적 재계획)

오류 발생 시 계획을 동적으로 수정합니다.

**재계획 유형:**
| 유형 | 설명 | 사용 시점 |
|------|------|----------|
| `refine` | 현재 step의 코드만 수정 | 구문 오류, 작은 버그 |
| `insert_steps` | 누락된 step 추가 | 전처리 누락, 의존성 누락 |
| `replace_step` | step 전체 교체 | 접근 방식 변경 필요 |
| `replan_remaining` | 남은 모든 step 재계획 | 근본적인 방향 수정 |

**재계획 프롬프트에 포함되는 정보:**
- 현재까지 실행된 step들
- 발생한 오류 메시지
- 노트북 현재 상태
- 사용 가능한 변수/함수

**코드 위치:** `backend/prompts/auto_agent_prompts.py` → `format_replan_prompt()`

---

## Frontend 상태 머신

```
┌────────────┐     ┌─────────────┐     ┌───────────┐
│  planning  │ ──▶ │  validating │ ──▶ │ executing │
└────────────┘     └─────────────┘     └───────────┘
                                             │
                   ┌─────────────┐           │
                   │  replanning │ ◀─────────┤
                   └─────────────┘           │
                          │                  ▼
                          │         ┌─────────────┐
                          └───────▶ │  reflecting │
                                    └─────────────┘
                                           │
                          ┌────────────────┴────────────────┐
                          ▼                                 ▼
                   ┌───────────┐                     ┌──────────┐
                   │ completed │                     │  failed  │
                   └───────────┘                     └──────────┘
```

**코드 위치:** `frontend/services/AgentOrchestrator.ts`

---

## 핵심 파일 위치

| 컴포넌트 | 파일 경로 |
|----------|----------|
| API 핸들러 | `backend/handlers/auto_agent.py` |
| 프롬프트 템플릿 | `backend/prompts/auto_agent_prompts.py` |
| Knowledge Base | `backend/knowledge/loader.py` |
| 라이브러리 가이드 | `backend/knowledge/libraries/*.md` |
| 코드 검증기 | `backend/services/code_validator.py` |
| Reflection 엔진 | `backend/services/reflection_engine.py` |
| Frontend 오케스트레이터 | `frontend/services/AgentOrchestrator.ts` |

---

## 아키텍처 특징

1. **Self-Healing**: 오류 발생 시 자동으로 코드 수정 시도
2. **Context-Aware**: 노트북 상태를 지속적으로 추적
3. **Knowledge-Enhanced**: 라이브러리별 전문 지식 동적 로딩
4. **Fail-Fast Validation**: 실행 전 코드 품질 사전 검증
5. **Adaptive Planning**: 상황에 따른 유연한 계획 수정
