...
HDSP Agent는 **Agent Server 분리 아키텍처**를 채택하며, 두 가지 통신 경로를 사용합니다:
...
│  │  /hdsp-agent/* → Agent Server 포워딩 (비즈니스 로직 없음)                           │  │
│  └────────────────────────────────┬──────────────────────────────────────────────────┘  │
└───────────────────────────────────┼─────────────────────────────────────────────────────┘
                                    │ HTTP/REST (Port 8000)
                                    ▼
                    ...
                    │  ├─▶ ErrorClassifier (결정론적)  │
                    │  └─▶ StateVerifier (결정론적)    │
                    └───────────────────────────────────┘
```

**통신 경로:**
- **REST API (프록시 경유)**: ApiService → jupyter_ext → Agent Server (계획/검증/리플랜)
