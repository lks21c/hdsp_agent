# HDSP Agent 인수인계 문서

> 마지막 업데이트: 2025-11-25
> 버전: 0.1.0

---

## 📋 목차

1. [프로젝트 개요](#1-프로젝트-개요)
2. [Backend 디렉토리 구조](#2-backend-디렉토리-구조)
3. [Frontend 디렉토리 구조](#3-frontend-디렉토리-구조)
4. [파일별 상세 역할](#4-파일별-상세-역할)
5. [빌드 프로세스](#5-빌드-프로세스)
6. [Wheel 파일 구조](#6-wheel-파일-구조)
7. [JupyterHub on K8s 배포](#7-jupyterhub-on-k8s-배포)
8. [설치 검증](#8-설치-검증)
9. [설정 방법](#9-설정-방법)
10. [트러블슈팅](#10-트러블슈팅)
11. [개발 가이드](#11-개발-가이드)

---

## 1. 프로젝트 개요

### 1.1 소개

**HDSP Agent**는 JupyterLab 4.0+ 환경에서 AI 기반 코드 지원을 제공하는 Extension입니다.

**주요 기능:**
- 🤖 **채팅 인터페이스**: 사이드바에서 LLM과 대화
- 📝 **셀 수준 분석**: 코드 설명(Explain), 오류 수정(Fix), 커스텀 프롬프트
- 📓 **노트북 생성**: 프롬프트 기반 자동 노트북 생성
- 💾 **저장 인터셉션**: 노트북 저장 전 분석 및 검토

### 1.2 기술 스택

| 레이어 | 기술 | 버전 |
|--------|------|------|
| **Frontend** | TypeScript + React | TS 5.1.6, React 18.2 |
| **UI Framework** | Material-UI + Emotion | MUI 5.14 |
| **JupyterLab** | JupyterLab Extensions API | 4.0+ |
| **Backend** | Python + Jupyter Server | Python 3.8+, Jupyter Server 2.0+ |
| **HTTP** | aiohttp (async) | 3.8+ |
| **LLM Providers** | Gemini, OpenAI, vLLM | - |

### 1.3 아키텍처 개요

```
┌─────────────────────────────────────────────────────────┐
│                   JupyterLab UI                         │
│  ┌─────────────┐  ┌──────────────┐  ┌────────────────┐ │
│  │ Sidebar     │  │ Cell Buttons │  │ Save           │ │
│  │ Chat Panel  │  │ (E/F/?)      │  │ Interceptor    │ │
│  └─────────────┘  └──────────────┘  └────────────────┘ │
└────────────────────────┬────────────────────────────────┘
                         │ REST API (/hdsp-agent/*)
┌────────────────────────▼────────────────────────────────┐
│              Jupyter Server Extension                    │
│  ┌──────────────────────────────────────────────────┐   │
│  │ Handlers (10 endpoints)                          │   │
│  │  - ChatHandler (일반/스트리밍)                   │   │
│  │  - CellActionHandler (explain/fix/custom)        │   │
│  │  - ConfigHandler (설정 관리)                     │   │
│  └──────────────────────────────────────────────────┘   │
│  ┌──────────────────────────────────────────────────┐   │
│  │ LLMService                                        │   │
│  │  - Gemini API (60s timeout, retry logic)         │   │
│  │  - OpenAI API (GPT-4)                            │   │
│  │  - vLLM API (local inference)                    │   │
│  └──────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────┘
```

---

## 2. Backend 디렉토리 구조

### 2.1 전체 구조

```
backend/
├── __init__.py                    # Extension 진입점
├── _version.py                    # 버전 정보
├── llm_service.py                 # 핵심 LLM 통합 서비스
├── handlers/                      # REST API 핸들러
│   ├── __init__.py                # 라우트 등록
│   ├── base.py                    # 기본 핸들러 클래스
│   ├── chat.py                    # 채팅 API (일반 + 스트리밍)
│   ├── config.py                  # 설정 관리 API
│   ├── cell_action.py             # 셀 분석 API
│   ├── status.py                  # 상태 확인 API
│   ├── test_llm.py                # LLM 테스트 API
│   └── notebook_generation.py     # 노트북 생성 API
├── services/                      # 비즈니스 로직
│   ├── __init__.py
│   ├── config_manager.py          # 설정 파일 관리 (싱글톤)
│   ├── llm_client.py              # LLM 클라이언트 래퍼
│   ├── notebook_generator.py      # 노트북 생성 로직
│   ├── prompt_builder.py          # 프롬프트 구축
│   └── task_manager.py            # 백그라운드 작업 관리
├── etc/                           # 설정 파일
│   └── jupyter/
│       └── jupyter_server_config.d/
│           └── hdsp_agent.json    # Jupyter Server 자동 등록
└── labextension/                  # 빌드된 frontend (자동 생성)
    ├── package.json
    ├── static/
    │   ├── lib_index_js.*.js      # 메인 플러그인
    │   ├── remoteEntry.*.js       # Module Federation
    │   ├── style_index_css.*.js   # 스타일
    │   └── vendors-*.js           # React, MUI 등
    └── build_log.json
```

### 2.2 핵심 파일 요약

| 파일 | 역할 | 중요도 |
|------|------|--------|
| `__init__.py` | Extension 등록, handler 초기화 | ⭐⭐⭐ |
| `llm_service.py` | 3개 LLM provider 통합 | ⭐⭐⭐ |
| `handlers/__init__.py` | 10개 API 라우트 등록 | ⭐⭐⭐ |
| `handlers/chat.py` | 채팅 메시지 처리 (일반/스트리밍) | ⭐⭐⭐ |
| `services/config_manager.py` | 설정 저장/로드 (JSON 파일) | ⭐⭐ |
| `services/task_manager.py` | 비동기 작업 관리 | ⭐⭐ |

---

## 3. Frontend 디렉토리 구조

### 3.1 전체 구조

```
frontend/
├── index.ts                       # Extension manifest (4개 플러그인 export)
├── plugins/                       # JupyterLab 플러그인
│   ├── sidebar-plugin.ts          # 사이드바 패널 생성
│   ├── cell-buttons-plugin.ts     # 셀에 E/F/? 버튼 주입
│   ├── prompt-generation-plugin.tsx # 커스텀 프롬프트 다이얼로그
│   └── save-interceptor-plugin.ts # 노트북 저장 전 분석
├── components/                    # React 컴포넌트
│   ├── AgentPanel.tsx             # 메인 채팅 UI (900줄+)
│   ├── SettingsPanel.tsx          # LLM 설정 UI
│   ├── PromptGenerationDialog.tsx # 프롬프트 입력 다이얼로그
│   └── TaskProgressWidget.tsx     # 작업 진행 상태 표시
├── services/                      # API 클라이언트
│   ├── ApiService.ts              # REST API 통신
│   └── TaskService.ts             # 작업 추적
├── utils/                         # 유틸리티
│   └── markdownRenderer.ts        # Markdown → HTML 변환
├── types/                         # TypeScript 타입 정의
│   └── index.ts                   # 인터페이스 및 Enum
├── styles/                        # 스타일시트
│   └── *.css
└── svg.d.ts                       # SVG import 타입
```

### 3.2 플러그인 로딩 순서

**중요**: `frontend/index.ts`에서 플러그인이 다음 순서로 export되며, 이 순서가 중요합니다:

1. **sidebarPlugin** - 사이드바 패널 생성 (먼저 생성되어야 함)
2. **cellButtonsPlugin** - 셀 버튼 주입 (사이드바 참조 필요)
3. **promptGenerationPlugin** - 프롬프트 다이얼로그
4. **saveInterceptorPlugin** - 저장 인터셉션

---

## 4. 파일별 상세 역할

### 4.1 Backend 핵심 파일

#### 4.1.1 `backend/__init__.py`

**역할**: Extension 진입점 및 등록

```python
def _jupyter_labextension_paths():
    """JupyterLab에 extension 등록"""
    return [{'src': 'labextension', 'dest': '@hdsp-agent/extension'}]

def _jupyter_server_extension_points():
    """Jupyter Server에 extension 등록"""
    return [{'module': 'backend'}]

def load_jupyter_server_extension(server_app):
    """서버 시작 시 handler 초기화"""
    from .handlers import setup_handlers
    web_app = server_app.web_app
    setup_handlers(web_app)  # 모든 API 라우트 등록
```

**중요 포인트**:
- Jupyter Server가 시작할 때 자동 호출
- `setup_handlers()`를 통해 10개 API 엔드포인트 등록
- ConfigManager를 싱글톤으로 초기화하여 `web_app.settings`에 저장

#### 4.1.2 `backend/llm_service.py`

**역할**: 3개 LLM provider 통합 서비스

**지원 Provider**:
1. **Google Gemini** (기본)
   - 모델: `gemini-2.5-pro`
   - Timeout: 60s
   - Retry 로직: 503/429 에러 시 exponential backoff
   - 최대 출력 토큰: 4096

2. **OpenAI**
   - 모델: `gpt-4`, `gpt-3.5-turbo`
   - 최대 출력 토큰: 2000

3. **vLLM** (로컬 inference)
   - OpenAI 호환 API
   - 기본 endpoint: `http://localhost:8000`

**주요 메서드**:

```python
class LLMService:
    async def generate_response(self, prompt: str, context: str = None) -> str:
        """단일 응답 생성 (블로킹)"""

    async def generate_response_stream(self, prompt: str, context: str = None):
        """스트리밍 응답 생성 (AsyncGenerator)"""
        # SSE로 실시간 청크 전송
```

**Retry 로직**:
- **503 (Overload)**: 2초, 4초, 8초 대기 후 재시도 (최대 3회)
- **429 (Rate Limit)**: 5초, 10초, 20초 대기 후 재시도 (최대 3회)

#### 4.1.3 `backend/handlers/__init__.py`

**역할**: API 라우트 등록

**등록된 엔드포인트** (10개):

| Method | 경로 | Handler | 용도 |
|--------|------|---------|------|
| POST | `/hdsp-agent/config` | ConfigHandler | 설정 저장 |
| GET | `/hdsp-agent/config` | ConfigHandler | 설정 조회 |
| POST | `/hdsp-agent/cell/action` | CellActionHandler | 셀 분석 |
| GET | `/hdsp-agent/status` | StatusHandler | 상태 확인 |
| POST | `/hdsp-agent/chat/message` | ChatHandler | 채팅 (일반) |
| POST | `/hdsp-agent/chat/stream` | ChatStreamHandler | 채팅 (스트리밍) |
| POST | `/hdsp-agent/test-llm` | TestLLMHandler | LLM 연결 테스트 |
| POST | `/hdsp-agent/notebook/generate` | NotebookGenerationHandler | 노트북 생성 |
| GET | `/hdsp-agent/task/{id}/status` | TaskStatusHandler | 작업 상태 조회 |
| GET | `/hdsp-agent/task/{id}/stream` | TaskStatusStreamHandler | 작업 진행 스트리밍 |
| POST | `/hdsp-agent/task/{id}/cancel` | TaskCancelHandler | 작업 취소 |

#### 4.1.4 `backend/handlers/chat.py`

**역할**: 채팅 메시지 처리

**`ChatHandler` (일반 응답)**:
- 요청: `{message: string, conversationId?: string}`
- 응답: `{conversationId, messageId, content, metadata}`
- 대화 기록: 메모리에 저장 (최근 5개 메시지를 context로 사용)

**`ChatStreamHandler` (스트리밍 응답)**:
- SSE (Server-Sent Events) 프로토콜 사용
- 실시간 청크 전송: `data: {"content": "...", "done": false}\n\n`
- 완료 시: `data: {"content": "", "done": true, "metadata": {...}}\n\n`

**대화 컨텍스트**:
```python
# 최근 5개 메시지를 컨텍스트로 포함
context = "\n".join([
    f"{'User' if msg['role'] == 'user' else 'Assistant'}: {msg['content']}"
    for msg in history[-5:]
])
```

#### 4.1.5 `backend/handlers/cell_action.py`

**역할**: 셀 분석 요청 처리

**지원 액션**:
1. **explain**: 코드 설명
2. **fix**: 오류 수정 제안
3. **custom**: 커스텀 프롬프트

**요청 형식**:
```json
{
  "cellId": "cell-123",
  "cellContent": "def foo():\n    print('hello')",
  "action": "explain",
  "customPrompt": "이 코드를 최적화해줘" // custom 액션 시
}
```

**응답 형식**:
```json
{
  "cellId": "cell-123",
  "response": "이 함수는...",
  "metadata": {
    "model": "gemini-2.5-pro",
    "tokens": 150,
    "duration": 1250  // ms
  },
  "fixedCode": "def foo():\n    ..."  // fix 액션 시
}
```

#### 4.1.6 `backend/services/config_manager.py`

**역할**: 설정 파일 관리 (싱글톤 패턴)

**저장 위치**: `~/.jupyter/hdsp_agent_config.json`

**기본 설정**:
```json
{
  "provider": "gemini",
  "gemini": {
    "apiKey": "",
    "model": "gemini-2.5-pro"
  },
  "vllm": {
    "endpoint": "http://localhost:8000",
    "apiKey": "",
    "model": "meta-llama/Llama-2-7b-chat-hf"
  },
  "openai": {
    "apiKey": "",
    "model": "gpt-4"
  }
}
```

**주요 메서드**:
```python
class ConfigManager:
    @classmethod
    def get_instance(cls): ...  # 싱글톤 인스턴스

    def get_config(self) -> dict:
        """설정 로드 (파일에서 최신 상태 읽기)"""

    def save_config(self, config: dict):
        """설정 저장 (파일에 JSON 쓰기)"""
```

#### 4.1.7 `backend/services/task_manager.py`

**역할**: 백그라운드 작업 관리 (비동기 싱글톤)

**작업 상태**:
- `PENDING` → `RUNNING` → `COMPLETED` / `FAILED` / `CANCELLED`

**Task 모델**:
```python
{
    "task_id": "uuid",
    "status": "running",
    "progress": 75,  # 0-100
    "message": "노트북 셀 생성 중...",
    "result": None,  # 완료 시 결과
    "error": None,   # 실패 시 에러 메시지
    "created_at": "2025-11-25T10:00:00",
    "updated_at": "2025-11-25T10:01:30"
}
```

**주요 메서드**:
```python
class TaskManager:
    async def create_task(self, description: str) -> str:
        """새 작업 생성, task_id 반환"""

    def update_task_progress(self, task_id: str, progress: int, message: str):
        """진행률 업데이트 (0-100)"""

    def complete_task(self, task_id: str, result: any):
        """작업 완료 처리"""

    def fail_task(self, task_id: str, error: str):
        """작업 실패 처리"""
```

### 4.2 Frontend 핵심 파일

#### 4.2.1 `frontend/index.ts`

**역할**: Extension manifest, 플러그인 export

```typescript
const plugins: JupyterFrontEndPlugin<any>[] = [
  sidebarPlugin,           // 1. 사이드바 패널
  cellButtonsPlugin,       // 2. 셀 버튼
  promptGenerationPlugin,  // 3. 프롬프트 다이얼로그
  saveInterceptorPlugin    // 4. 저장 인터셉션
];

export default plugins;
```

**중요**: 순서가 바뀌면 플러그인 간 참조가 깨질 수 있음!

#### 4.2.2 `frontend/plugins/sidebar-plugin.ts`

**역할**: 사이드바에 Agent 패널 추가

**주요 기능**:
1. `AgentPanelWidget` 생성 (React wrapper)
2. 오른쪽 사이드바에 추가 (rank: 100)
3. `hdsp-agent:toggle-sidebar` 커맨드 등록
4. 상태 복원 (선택적)

**코드 흐름**:
```typescript
// 1. Widget 생성
const widget = new AgentPanelWidget(apiService);
widget.id = 'hdsp-agent-sidebar';
widget.title.icon = agentIcon;

// 2. 사이드바 추가
app.shell.add(widget, 'right', { rank: 100 });

// 3. 커맨드 등록
app.commands.addCommand('hdsp-agent:toggle-sidebar', {
  execute: () => {
    if (!widget.isAttached) {
      app.shell.add(widget, 'right');
    }
    app.shell.activateById(widget.id);
  }
});
```

#### 4.2.3 `frontend/plugins/cell-buttons-plugin.ts`

**역할**: 코드 셀에 E/F/? 버튼 주입

**버튼 기능**:
- **E** (Explain): 코드 설명 요청
- **F** (Fix): 오류 수정 제안
- **?** (Custom): 커스텀 프롬프트 다이얼로그

**주입 로직**:
```typescript
// 1. 노트북 추적
tracker.widgetAdded.connect((sender, panel) => {
  const notebook = panel.content;

  // 2. 셀 변경 감지
  notebook.model.cells.changed.connect(() => {
    // 200ms 후 버튼 주입 (렌더링 대기)
    setTimeout(() => injectButtons(notebook), 200);
  });
});

// 3. 버튼 클릭 핸들러
buttonE.onclick = () => {
  const cellContent = cell.model.value.text;
  const cellId = cell.model.id;

  // AgentPanel로 전달 (ref 사용)
  agentPanelRef.current?.setInput(`[셀 ${cellIndex}] 설명해줘`);
  agentPanelRef.current?.setLlmPrompt(`Explain this code:\n${cellContent}`);
  agentPanelRef.current?.setCurrentCellId(cellId);
  agentPanelRef.current?.handleSendMessage();
};
```

**타이밍 이슈 해결**:
- 초기 주입: 200ms 대기
- 재시도: 500ms 후 (늦게 렌더링되는 셀 대응)

#### 4.2.4 `frontend/components/AgentPanel.tsx`

**역할**: 메인 채팅 UI (900줄+의 핵심 컴포넌트)

**상태 관리**:
```typescript
const [messages, setMessages] = useState<IChatMessage[]>([]);
const [input, setInput] = useState('');
const [isLoading, setIsLoading] = useState(false);
const [isStreaming, setIsStreaming] = useState(false);
const [conversationId, setConversationId] = useState<string>('');
const [showSettings, setShowSettings] = useState(false);
const [llmConfig, setLlmConfig] = useState<LLMConfig | null>(null);
```

**Ref로 노출된 메서드** (cell-buttons-plugin에서 호출):
```typescript
useImperativeHandle(ref, () => ({
  handleSendMessage: async () => { ... },
  setInput: (value: string) => { ... },
  setLlmPrompt: (prompt: string) => { ... },
  setCurrentCellId: (cellId: string) => { ... },
  setCurrentCellIndex: (cellIndex: number) => { ... }
}));
```

**스트리밍 처리**:
```typescript
const response = await fetch('/hdsp-agent/chat/stream', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({ message, conversationId })
});

const reader = response.body.getReader();
const decoder = new TextDecoder();

while (true) {
  const { done, value } = await reader.read();
  if (done) break;

  const chunk = decoder.decode(value);
  const lines = chunk.split('\n');

  for (const line of lines) {
    if (line.startsWith('data: ')) {
      const data = JSON.parse(line.slice(6));
      if (data.content) {
        // 실시간으로 메시지에 추가
        setMessages(prev => updateLastMessage(prev, data.content));
      }
    }
  }
}
```

**Markdown 렌더링**:
```typescript
import { formatMarkdownToHtml } from '../utils/markdownRenderer';

const messageHtml = formatMarkdownToHtml(message.content);
<div dangerouslySetInnerHTML={{ __html: messageHtml }} />
```

#### 4.2.5 `frontend/components/SettingsPanel.tsx`

**역할**: LLM 설정 UI

**Provider별 필드**:

**Gemini**:
```typescript
<TextField
  label="API Key"
  type="password"
  value={config.gemini.apiKey}
  onChange={e => updateConfig('gemini', 'apiKey', e.target.value)}
/>
<Select value={config.gemini.model}>
  <MenuItem value="gemini-2.5-pro">Gemini 2.5 Pro</MenuItem>
  <MenuItem value="gemini-pro">Gemini Pro</MenuItem>
</Select>
```

**OpenAI**:
```typescript
<TextField label="API Key" type="password" />
<Select value={config.openai.model}>
  <MenuItem value="gpt-4">GPT-4</MenuItem>
  <MenuItem value="gpt-3.5-turbo">GPT-3.5 Turbo</MenuItem>
</Select>
```

**vLLM**:
```typescript
<TextField
  label="Endpoint URL"
  value={config.vllm.endpoint}
  placeholder="http://localhost:8000"
/>
<TextField label="Model" value={config.vllm.model} />
```

**API 키 마스킹**:
- 표시: `****XXXX` (마지막 4자만 표시)
- 저장: masked 키는 기존 값 유지

#### 4.2.6 `frontend/services/ApiService.ts`

**역할**: REST API 클라이언트

**CSRF 토큰 처리**:
```typescript
private getCsrfToken(): string {
  const value = `; ${document.cookie}`;
  const parts = value.split(`; _xsrf=`);
  return parts.length === 2 ? parts.pop().split(';').shift() : '';
}

private getHeaders(): HeadersInit {
  return {
    'Content-Type': 'application/json',
    'X-XSRFToken': this.getCsrfToken()
  };
}
```

**주요 메서드**:
```typescript
class ApiService {
  async sendMessage(request: IChatRequest): Promise<IChatResponse> {
    const response = await fetch(`${this.baseUrl}/chat/message`, {
      method: 'POST',
      headers: this.getHeaders(),
      body: JSON.stringify(request)
    });
    return response.json();
  }

  async cellAction(request: ICellActionRequest): Promise<ICellResponse> { ... }
  async getConfig(): Promise<IAgentConfig> { ... }
  async saveConfig(config: IAgentConfig): Promise<void> { ... }
  async getStatus(): Promise<IHealthStatus> { ... }
}
```

---

## 5. 빌드 프로세스

### 5.1 빌드 파이프라인

```
┌─────────────────────────────────────────────────────┐
│ 1. TypeScript 컴파일                                 │
│    frontend/ → lib/ (tsc)                           │
│    - *.ts → *.js + *.d.ts                           │
│    - JSX → React.createElement() 변환                │
└─────────────────────────────────────────────────────┘
                      ↓
┌─────────────────────────────────────────────────────┐
│ 2. JupyterLab Extension 빌드                        │
│    lib/ → backend/labextension/ (webpack)           │
│    - Module Federation으로 번들링                   │
│    - CSS 추출 및 최적화                             │
│    - Code splitting (vendors, main, styles)         │
└─────────────────────────────────────────────────────┘
                      ↓
┌─────────────────────────────────────────────────────┐
│ 3. Python Wheel 생성                                │
│    backend/ + pyproject.toml → dist/*.whl          │
│    - Python 파일 포함                               │
│    - labextension/ 포함                             │
│    - 메타데이터 생성                                │
└─────────────────────────────────────────────────────┘
```

### 5.2 빌드 명령어

#### 자동 빌드 (권장)

```bash
cd <your-git-repo>/hdsp_agent

# 전체 빌드 스크립트
./build.sh
```

**`build.sh` 내용**:
```bash
#!/bin/bash
set -e

echo "🔨 Step 1: TypeScript 컴파일..."
npx tsc

echo "📦 Step 2: JupyterLab Extension 빌드..."
poetry run jupyter labextension build .

echo "🎁 Step 3: Wheel 파일 생성..."
poetry build

echo "✅ 빌드 완료!"
ls -lh dist/
```

#### 수동 빌드 (단계별)

```bash
# Step 1: TypeScript 컴파일
npx tsc
# 또는
jlpm build:lib

# Step 2: Extension 빌드
poetry run jupyter labextension build .

# Step 3: Wheel 생성
poetry build
```

### 5.3 개발 모드 빌드

```bash
# Watch mode (코드 변경 시 자동 재빌드)
jlpm watch

# 또는 개별 watch
jlpm watch:src          # TypeScript watch
jlpm watch:labextension  # Extension watch
```

### 5.4 빌드 출력물

```
dist/
└── hdsp_agent-0.1.0-py3-none-any.whl   # 배포용 wheel 파일

backend/labextension/                    # 빌드된 extension
├── package.json
├── static/
│   ├── 179.*.js                        # Async chunk
│   ├── lib_index_js.*.js               # Main plugin
│   ├── remoteEntry.*.js                # Module Federation
│   ├── style_index_css.*.js            # Styles
│   ├── style.js                        # Style entry
│   └── vendors-*.js                    # React, MUI, dependencies
└── build_log.json

lib/                                     # TypeScript 컴파일 결과
├── index.js, index.d.ts
├── components/
├── plugins/
└── services/
```

### 5.5 빌드 설정 파일

#### `tsconfig.json`

```json
{
  "compilerOptions": {
    "rootDir": "frontend",      // 소스 디렉토리
    "outDir": "lib",            // 출력 디렉토리
    "target": "ES2020",
    "module": "esnext",
    "jsx": "react",
    "declaration": true,        // .d.ts 생성
    "skipLibCheck": true
  },
  "include": ["frontend/**/*"]
}
```

#### `package.json` (jupyterlab 섹션)

```json
{
  "jupyterlab": {
    "extension": true,
    "outputDir": "backend/labextension",  // Extension 출력 경로
    "sharedPackages": {
      "@jupyterlab/application": {
        "bundled": false,       // JupyterLab 제공 패키지 재사용
        "singleton": true
      }
    }
  }
}
```

#### `pyproject.toml` (build 섹션)

```toml
[tool.poetry]
packages = [{ include = "backend" }]  # backend/ 포함

include = [
    "backend/labextension/**/*",      # 빌드된 extension
    "backend/etc/**/*",               # 설정 파일
    "install.json"                    # 메타데이터
]

[tool.jupyter-packaging.builder]
factory = "jupyter_packaging.npm_builder"

[tool.jupyter-packaging.build-args]
build_cmd = "build:prod"
npm = ["jlpm"]
source_dir = "frontend"
build_dir = "backend/labextension"
```

---

## 6. Wheel 파일 구조

### 6.1 Wheel 내부 구조

```
hdsp_agent-0.1.0-py3-none-any.whl (압축 파일)
│
├── backend/                           # Python 패키지
│   ├── __init__.py
│   ├── _version.py
│   ├── llm_service.py
│   │
│   ├── handlers/
│   │   ├── __init__.py
│   │   ├── base.py
│   │   ├── chat.py
│   │   ├── config.py
│   │   ├── cell_action.py
│   │   ├── status.py
│   │   ├── test_llm.py
│   │   └── notebook_generation.py
│   │
│   ├── services/
│   │   ├── __init__.py
│   │   ├── config_manager.py
│   │   ├── llm_client.py
│   │   ├── notebook_generator.py
│   │   ├── prompt_builder.py
│   │   └── task_manager.py
│   │
│   ├── etc/
│   │   └── jupyter/
│   │       └── jupyter_server_config.d/
│   │           └── hdsp_agent.json
│   │
│   └── labextension/                 # ⭐ 빌드된 Frontend
│       ├── package.json
│       ├── static/
│       │   ├── 179.*.js             # Async chunk
│       │   ├── lib_index_js.*.js    # Main plugin (40KB)
│       │   ├── remoteEntry.*.js     # Module Federation (2KB)
│       │   ├── style_index_css.*.js # Styles (15KB)
│       │   ├── style.js             # Style entry
│       │   └── vendors-*.js         # React, MUI (500KB+)
│       └── build_log.json
│
├── hdsp_agent-0.1.0.dist-info/      # 메타데이터
│   ├── METADATA
│   ├── WHEEL
│   ├── RECORD
│   └── entry_points.txt             # Extension 등록 정보
│
└── install.json                      # JupyterLab extension 메타데이터
```

### 6.2 핵심 파일 설명

#### `backend/labextension/static/`

**Module Federation 구조**:
- **remoteEntry.*.js**: Extension 진입점
- **lib_index_js.*.js**: 4개 플러그인 코드
- **vendors-*.js**: React, MUI, Emotion 등 공통 라이브러리
- **style_index_css.*.js**: 모든 CSS 번들

#### `hdsp_agent-0.1.0.dist-info/entry_points.txt`

```ini
[jupyter_labextensions]
hdsp-agent = backend

[jupyter_server.extension_points]
hdsp-agent = backend:_jupyter_server_extension_points
```

**역할**: Jupyter가 extension을 자동 발견하고 로드

#### `install.json`

```json
{
  "packageManager": "python",
  "packageName": "@hdsp-agent/extension",
  "uninstallInstructions": "Use `pip uninstall hdsp-agent` to remove"
}
```

### 6.3 설치 과정 상세

```bash
pip install hdsp_agent-0.1.0-py3-none-any.whl
```

**내부 동작**:

1. **Python 파일 설치**:
   ```
   site-packages/backend/
   ├── __init__.py
   ├── handlers/
   └── services/
   ```

2. **Extension 파일 복사**:
   ```
   site-packages/backend/labextension/
   └── static/
       ├── lib_index_js.*.js
       └── vendors-*.js
   ```

3. **설정 파일 등록**:
   ```
   ~/.jupyter/jupyter_server_config.d/hdsp_agent.json
   ```

4. **Entry points 등록**:
   - `jupyter_labextensions`: Frontend extension
   - `jupyter_server.extension_points`: Backend extension

5. **Jupyter 재시작 시**:
   - Server가 `backend:_jupyter_server_extension_points()` 호출
   - `load_jupyter_server_extension()` 실행 → API handlers 등록
   - JupyterLab이 `backend/labextension/` 로드 → 플러그인 활성화

---

## 7. JupyterHub on K8s 배포

### 7.1 사전 준비

#### 로컬에서 Wheel 빌드

```bash
cd <your-git-repo>/hdsp_agent

# 전체 빌드
./build.sh

# 결과 확인
ls -lh dist/
# hdsp_agent-0.1.0-py3-none-any.whl (약 600KB)
```

### 7.2 배포 방법 1: Docker 이미지에 포함 (권장)

#### Dockerfile

```dockerfile
FROM jupyter/minimal-notebook:latest

# root 권한으로 전환
USER root

# Wheel 파일 복사
COPY dist/hdsp_agent-0.1.0-py3-none-any.whl /tmp/

# Extension 설치
RUN pip install --no-cache-dir /tmp/hdsp_agent-0.1.0-py3-none-any.whl

# Extension 활성화 확인
RUN jupyter server extension list && \
    jupyter labextension list

# 임시 파일 삭제
RUN rm /tmp/hdsp_agent-0.1.0-py3-none-any.whl

# 다시 jovyan 사용자로
USER ${NB_UID}

# JupyterLab 시작
CMD ["jupyter", "lab", "--ip=0.0.0.0", "--port=8888", "--no-browser"]
```

#### 빌드 및 푸시

```bash
# Docker 이미지 빌드
docker build -t your-registry/jupyter-hdsp-agent:0.1.0 .

# Registry에 푸시
docker push your-registry/jupyter-hdsp-agent:0.1.0
```

#### JupyterHub Helm Values

```yaml
# values.yaml
singleuser:
  image:
    name: your-registry/jupyter-hdsp-agent
    tag: "0.1.0"

  # LLM API 키 설정 (Secret 사용)
  extraEnv:
    GEMINI_API_KEY:
      valueFrom:
        secretKeyRef:
          name: llm-api-keys
          key: gemini-api-key
```

### 7.3 배포 방법 2: InitContainer로 설치

#### ConfigMap에 Wheel 저장

```bash
# Wheel을 base64 인코딩
base64 dist/hdsp_agent-0.1.0-py3-none-any.whl > wheel.b64

# ConfigMap 생성
kubectl create configmap hdsp-agent-wheel \
  --from-file=hdsp_agent-0.1.0-py3-none-any.whl=wheel.b64 \
  -n jupyterhub
```

#### JupyterHub Helm Values

```yaml
# values.yaml
singleuser:
  storage:
    extraVolumes:
      - name: hdsp-wheel
        configMap:
          name: hdsp-agent-wheel
    extraVolumeMounts:
      - name: hdsp-wheel
        mountPath: /tmp/wheels

  initContainers:
    - name: install-hdsp-agent
      image: jupyter/minimal-notebook:latest
      command: ["/bin/bash", "-c"]
      args:
        - |
          base64 -d /tmp/wheels/hdsp_agent-0.1.0-py3-none-any.whl > /tmp/wheel.whl
          pip install --user /tmp/wheel.whl
          rm /tmp/wheel.whl
      volumeMounts:
        - name: hdsp-wheel
          mountPath: /tmp/wheels
        - name: home
          mountPath: /home/jovyan
```

**한계점**: User별로 extension 설치되며, 영구 스토리지 필요

### 7.4 배포 방법 3: PVC를 통한 공유 설치

#### Step 1: PVC 생성

```yaml
# wheel-pvc.yaml
apiVersion: v1
kind: PersistentVolumeClaim
metadata:
  name: hdsp-wheels-pvc
  namespace: jupyterhub
spec:
  accessModes:
    - ReadOnlyMany  # 여러 Pod에서 읽기
  resources:
    requests:
      storage: 1Gi
  storageClassName: standard
```

```bash
kubectl apply -f wheel-pvc.yaml
```

#### Step 2: Wheel 파일 업로드

```bash
# 임시 Pod로 업로드
kubectl run wheel-uploader \
  --image=alpine \
  --restart=Never \
  --rm -i \
  --overrides='
{
  "apiVersion": "v1",
  "spec": {
    "containers": [{
      "name": "uploader",
      "image": "alpine",
      "command": ["sleep", "3600"],
      "volumeMounts": [{
        "name": "wheels",
        "mountPath": "/wheels"
      }]
    }],
    "volumes": [{
      "name": "wheels",
      "persistentVolumeClaim": {
        "claimName": "hdsp-wheels-pvc"
      }
    }]
  }
}'

# 파일 복사
kubectl cp dist/hdsp_agent-0.1.0-py3-none-any.whl \
  jupyterhub/wheel-uploader:/wheels/

# Pod 삭제
kubectl delete pod wheel-uploader -n jupyterhub
```

#### Step 3: JupyterHub Helm Values

```yaml
# values.yaml
singleuser:
  storage:
    extraVolumes:
      - name: shared-wheels
        persistentVolumeClaim:
          claimName: hdsp-wheels-pvc
          readOnly: true
    extraVolumeMounts:
      - name: shared-wheels
        mountPath: /shared/wheels
        readOnly: true

  initContainers:
    - name: install-hdsp-agent
      image: jupyter/minimal-notebook:latest
      command: ["/bin/bash", "-c"]
      args:
        - pip install --user /shared/wheels/hdsp_agent-0.1.0-py3-none-any.whl
      volumeMounts:
        - name: shared-wheels
          mountPath: /shared/wheels
        - name: home
          mountPath: /home/jovyan
```

### 7.5 배포 방법 4: Sidecar Container (동적 설치)

```yaml
# values.yaml
singleuser:
  extraContainers:
    - name: extension-installer
      image: jupyter/minimal-notebook:latest
      command: ["/bin/bash", "-c"]
      args:
        - |
          while true; do
            if [ ! -f /home/jovyan/.hdsp-installed ]; then
              pip install --user https://your-cdn.com/hdsp_agent-0.1.0-py3-none-any.whl
              touch /home/jovyan/.hdsp-installed
            fi
            sleep 300
          done
      volumeMounts:
        - name: home
          mountPath: /home/jovyan
```

### 7.6 API 키 관리 (Secret)

#### Secret 생성

```bash
# Gemini API 키 설정
kubectl create secret generic llm-api-keys \
  --from-literal=gemini-api-key=YOUR_GEMINI_API_KEY \
  --from-literal=openai-api-key=YOUR_OPENAI_API_KEY \
  -n jupyterhub
```

#### Extension에서 사용

**방법 1: 환경변수로 주입**

```yaml
# values.yaml
singleuser:
  extraEnv:
    GEMINI_API_KEY:
      valueFrom:
        secretKeyRef:
          name: llm-api-keys
          key: gemini-api-key
```

사용자가 Settings에서 환경변수를 API 키로 사용하도록 안내

**방법 2: 기본 설정 파일 제공**

```yaml
singleuser:
  lifecycleHooks:
    postStart:
      exec:
        command:
          - /bin/bash
          - -c
          - |
            mkdir -p ~/.jupyter
            cat > ~/.jupyter/hdsp_agent_config.json <<EOF
            {
              "provider": "gemini",
              "gemini": {
                "apiKey": "$GEMINI_API_KEY",
                "model": "gemini-2.5-pro"
              }
            }
            EOF
```

### 7.7 Helm 배포 명령어

```bash
# Helm repo 추가
helm repo add jupyterhub https://hub.jupyter.org/helm-chart/
helm repo update

# JupyterHub 설치/업데이트
helm upgrade --install jupyterhub jupyterhub/jupyterhub \
  --namespace jupyterhub \
  --create-namespace \
  --values values.yaml \
  --version=3.0.0
```

### 7.8 배포 후 확인

```bash
# Pod 상태 확인
kubectl get pods -n jupyterhub

# Extension 설치 확인
kubectl exec -it jupyter-username -n jupyterhub -- \
  jupyter server extension list

# Extension 활성화 확인
kubectl exec -it jupyter-username -n jupyterhub -- \
  jupyter labextension list

# 로그 확인
kubectl logs -f jupyter-username -n jupyterhub
```

---

## 8. 설치 검증

### 8.1 Extension 등록 확인

```bash
# Jupyter Server Extension 확인
jupyter server extension list

# 정상 출력:
# Config dir: /Users/username/.jupyter
#     backend enabled
#     - Validating backend...
#       backend 0.1.0 OK
```

```bash
# JupyterLab Extension 확인
jupyter labextension list

# 정상 출력:
# JupyterLab v4.x.x
# /path/to/labextensions
#     @hdsp-agent/extension v0.1.0 enabled OK
```

### 8.2 JupyterLab UI 확인

1. **JupyterLab 시작**:
   ```bash
   jupyter lab
   ```

2. **사이드바 패널 확인**:
   - 오른쪽 사이드바에 🤖 아이콘 표시
   - 클릭 시 Agent 패널 열림
   - 채팅 입력창 및 설정 버튼 확인

3. **셀 버튼 확인**:
   - 노트북 생성
   - 코드 셀 추가
   - 셀 오른쪽 상단에 E/F/? 버튼 표시 확인

4. **설정 패널 확인**:
   - 톱니바퀴 아이콘 클릭
   - Provider 선택 (Gemini/OpenAI/vLLM)
   - API 키 입력 필드 확인

### 8.3 API 엔드포인트 테스트

```bash
# 터미널에서 curl 테스트

# 1. Status 확인
curl http://localhost:8888/hdsp-agent/status

# 정상 출력:
# {"status": "ok", "version": "0.1.0"}

# 2. Config 조회
curl http://localhost:8888/hdsp-agent/config

# 정상 출력:
# {"provider": "gemini", "gemini": {...}, ...}
```

### 8.4 LLM 연결 테스트

1. **설정에서 API 키 입력**:
   - Settings 열기
   - Provider 선택 (예: Gemini)
   - API 키 입력
   - Save 클릭

2. **채팅 테스트**:
   - "Hello" 입력 후 전송
   - 응답 수신 확인

3. **셀 분석 테스트**:
   - 코드 셀에 간단한 코드 작성
   - E 버튼 클릭
   - 설명 응답 확인

### 8.5 로그 확인

```bash
# JupyterLab 로그 (터미널에서 실행 시)
# 정상적으로 로드된 경우:
[ServerApp] hdsp_agent extension loaded
[ServerApp] Handlers registered: /hdsp-agent/*

# 브라우저 콘솔 로그 (F12)
# 정상적으로 로드된 경우:
[AgentPanel] Config loaded: {provider: 'gemini', ...}
[AgentPanel] Extension initialized
```

---

## 9. 설정 방법

### 9.1 설정 파일 위치

```
~/.jupyter/hdsp_agent_config.json
```

### 9.2 기본 설정 구조

```json
{
  "provider": "gemini",
  "gemini": {
    "apiKey": "YOUR_API_KEY_HERE",
    "model": "gemini-2.5-pro"
  },
  "vllm": {
    "endpoint": "http://localhost:8000",
    "apiKey": "",
    "model": "meta-llama/Llama-2-7b-chat-hf"
  },
  "openai": {
    "apiKey": "",
    "model": "gpt-4"
  }
}
```

### 9.3 Provider별 설정 가이드

#### Gemini (Google)

1. **API 키 발급**:
   - https://makersuite.google.com/app/apikey 방문
   - API key 생성

2. **설정**:
   ```json
   {
     "provider": "gemini",
     "gemini": {
       "apiKey": "AIzaSy...",
       "model": "gemini-2.5-pro"
     }
   }
   ```

3. **지원 모델**:
   - `gemini-2.5-pro` (권장)
   - `gemini-pro`
   - `gemini-1.5-flash`

#### OpenAI

1. **API 키 발급**:
   - https://platform.openai.com/api-keys 방문
   - API key 생성

2. **설정**:
   ```json
   {
     "provider": "openai",
     "openai": {
       "apiKey": "sk-...",
       "model": "gpt-4"
     }
   }
   ```

3. **지원 모델**:
   - `gpt-4` (권장)
   - `gpt-4-turbo`
   - `gpt-3.5-turbo`

#### vLLM (로컬)

1. **vLLM 서버 시작**:
   ```bash
   pip install vllm

   python -m vllm.entrypoints.openai.api_server \
     --model meta-llama/Llama-2-7b-chat-hf \
     --port 8000
   ```

2. **설정**:
   ```json
   {
     "provider": "vllm",
     "vllm": {
       "endpoint": "http://localhost:8000",
       "apiKey": "",  # 선택사항
       "model": "meta-llama/Llama-2-7b-chat-hf"
     }
   }
   ```

### 9.4 UI에서 설정 변경

1. JupyterLab 사이드바에서 Agent 패널 열기
2. 톱니바퀴 아이콘(⚙️) 클릭
3. Provider 선택
4. 해당 Provider 정보 입력
5. Save 클릭

### 9.5 환경변수로 설정 (K8s)

```yaml
# values.yaml
singleuser:
  extraEnv:
    HDSP_PROVIDER: "gemini"
    GEMINI_API_KEY:
      valueFrom:
        secretKeyRef:
          name: llm-api-keys
          key: gemini-api-key
```

Extension에서 환경변수 읽도록 수정 필요 시:

```python
# backend/services/config_manager.py
import os

def _default_config(self):
    return {
        'provider': os.getenv('HDSP_PROVIDER', 'gemini'),
        'gemini': {
            'apiKey': os.getenv('GEMINI_API_KEY', ''),
            'model': os.getenv('GEMINI_MODEL', 'gemini-2.5-pro')
        }
    }
```

---

## 10. 트러블슈팅

### 10.1 Extension이 로드되지 않을 때

**증상**: 사이드바에 Agent 패널이 없음

**해결 방법**:

1. **Extension 설치 확인**:
   ```bash
   jupyter labextension list
   ```

   `@hdsp-agent/extension`이 없으면:
   ```bash
   pip install --force-reinstall hdsp_agent-0.1.0-py3-none-any.whl
   ```

2. **캐시 클리어**:
   ```bash
   jupyter lab clean
   jupyter lab build
   ```

3. **Browser 캐시 삭제**:
   - Ctrl+Shift+R (강제 새로고침)
   - F12 → Network → Disable cache

4. **로그 확인**:
   ```bash
   jupyter lab --debug
   ```

### 10.2 API 404 에러

**증상**: 채팅 전송 시 "Failed to send message" 에러

**원인**: Backend handler가 등록되지 않음

**해결 방법**:

1. **Server Extension 확인**:
   ```bash
   jupyter server extension list
   ```

   `backend`가 없으면:
   ```bash
   jupyter server extension enable backend
   ```

2. **라우트 확인**:
   ```bash
   # JupyterLab 실행 후 터미널에서
   curl http://localhost:8888/hdsp-agent/status
   ```

   404 에러 시 서버 재시작:
   ```bash
   # JupyterLab 종료 후
   jupyter lab
   ```

3. **Backend 파일 확인**:
   ```bash
   python -c "import backend; print(backend.__file__)"
   # backend/__init__.py 경로 출력 확인
   ```

### 10.3 LLM 연결 실패

#### Gemini API 에러

**에러**: `503 Service Unavailable`

**해결**:
- Retry 로직이 자동 작동 (2s, 4s, 8s 대기)
- 여전히 실패 시 잠시 후 재시도

**에러**: `400 API key not valid`

**해결**:
- API 키 재확인
- https://makersuite.google.com/app/apikey 에서 새 키 발급

#### OpenAI API 에러

**에러**: `401 Unauthorized`

**해결**:
- API 키 형식 확인 (`sk-`로 시작)
- 계정에 크레딧 잔액 확인

**에러**: `429 Rate limit exceeded`

**해결**:
- Retry 로직 작동 (5s, 10s, 20s 대기)
- API usage limit 증가 요청

#### vLLM 연결 에러

**에러**: `Connection refused`

**해결**:
1. vLLM 서버 실행 확인:
   ```bash
   curl http://localhost:8000/v1/models
   ```

2. Endpoint URL 확인:
   - Settings에서 `http://localhost:8000` 입력
   - K8s 환경: Service DNS 사용 (예: `http://vllm-service:8000`)

### 10.4 빌드 에러

#### TypeScript 컴파일 에러

```
error TS2307: Cannot find module '@jupyterlab/application'
```

**해결**:
```bash
jlpm install  # 의존성 재설치
```

#### Extension 빌드 에러

```
ValueError: Please install nodejs >=12.0.0
```

**해결**:
```bash
# Node.js 버전 확인
node --version  # v18+ 권장

# nvm으로 업그레이드
nvm install 18
nvm use 18
```

#### Wheel 빌드 에러

```
ERROR: backend/labextension/static/style.js not found
```

**해결**:
```bash
# Extension 빌드 먼저 실행
jupyter labextension build .

# 그 후 wheel 빌드
poetry build
```

### 10.5 스트리밍 응답 끊김

**증상**: 채팅 응답이 중간에 멈춤

**원인**: Proxy timeout (Nginx, K8s Ingress 등)

**해결**:

**Nginx**:
```nginx
location /hdsp-agent/chat/stream {
    proxy_pass http://jupyterhub;
    proxy_buffering off;
    proxy_read_timeout 300s;
    proxy_connect_timeout 300s;
}
```

**K8s Ingress (Nginx)**:
```yaml
metadata:
  annotations:
    nginx.ingress.kubernetes.io/proxy-buffering: "off"
    nginx.ingress.kubernetes.io/proxy-read-timeout: "300"
    nginx.ingress.kubernetes.io/proxy-send-timeout: "300"
```

### 10.6 설정이 저장되지 않음

**증상**: Settings에서 저장해도 재시작 시 초기화

**원인**: 파일 권한 또는 경로 문제

**해결**:

1. **디렉토리 권한 확인**:
   ```bash
   ls -la ~/.jupyter/
   chmod 755 ~/.jupyter/
   ```

2. **설정 파일 직접 확인**:
   ```bash
   cat ~/.jupyter/hdsp_agent_config.json
   ```

3. **수동 생성**:
   ```bash
   mkdir -p ~/.jupyter
   cat > ~/.jupyter/hdsp_agent_config.json <<EOF
   {
     "provider": "gemini",
     "gemini": {
       "apiKey": "YOUR_KEY",
       "model": "gemini-2.5-pro"
     }
   }
   EOF
   ```

---

## 11. 개발 가이드

### 11.1 로컬 개발 환경 설정

#### 초기 설정

```bash
# 1. Repository clone
git clone https://github.com/lks21c/hdsp_agent.git
cd hdsp_agent

# 2. Python 환경 설정
poetry install

# 3. Node.js 의존성 설치
jlpm install

# 4. 개발 모드 설치
pip install -e .

# 5. Extension 개발 모드 활성화
jupyter labextension develop . --overwrite
```

### 11.2 Watch Mode 실행

**터미널 1**: TypeScript watch
```bash
jlpm watch:src
# frontend/ 변경 시 자동으로 lib/ 재빌드
```

**터미널 2**: Extension watch
```bash
jlpm watch:labextension
# lib/ 변경 시 자동으로 backend/labextension/ 재빌드
```

**터미널 3**: JupyterLab 실행
```bash
jupyter lab --autoreload
# Python 코드 변경 시 자동 재로드
```

### 11.3 코드 수정 워크플로우

#### Frontend 수정

1. `frontend/components/AgentPanel.tsx` 수정
2. Watch mode가 자동으로 `lib/components/AgentPanel.js` 생성
3. Extension watch가 자동으로 `backend/labextension/` 업데이트
4. 브라우저 새로고침 (Ctrl+R)

#### Backend 수정

1. `backend/handlers/chat.py` 수정
2. JupyterLab 자동 재로드 (--autoreload 옵션)
3. API 테스트

### 11.4 디버깅 방법

#### Frontend 디버깅

```typescript
// frontend/components/AgentPanel.tsx
console.log('[AgentPanel] Message sent:', message);
console.log('[AgentPanel] Response:', response);
```

**브라우저 콘솔** (F12):
- Console 탭에서 로그 확인
- Network 탭에서 API 요청/응답 확인
- Sources 탭에서 breakpoint 설정

#### Backend 디버깅

```python
# backend/handlers/chat.py
self.log.info(f"=== Chat message received ===")
self.log.info(f"Message: {message}")
```

**터미널 로그**:
```bash
jupyter lab --debug
# 모든 로그 출력
```

**Python 디버거**:
```python
import pdb; pdb.set_trace()
# 코드 실행 중단 및 디버깅
```

### 11.5 테스트

#### Frontend 테스트 (TODO)

```bash
# Jest 테스트 (아직 미구현)
jlpm test
```

#### Backend 테스트

```bash
# Pytest 실행
poetry run pytest tests/
```

#### E2E 테스트 (수동)

1. JupyterLab 실행
2. 노트북 생성
3. 코드 셀에서 E 버튼 클릭
4. 응답 확인
5. 채팅 입력 테스트
6. 설정 변경 테스트

### 11.6 코드 스타일

#### Python (Black + isort)

```bash
# 포맷팅
poetry run black backend/
poetry run isort backend/

# Lint
poetry run flake8 backend/
```

#### TypeScript (Prettier + ESLint)

```bash
# 포맷팅
jlpm prettier

# Lint
jlpm eslint

# 전체
jlpm lint
```

### 11.7 릴리스 프로세스

1. **버전 업데이트**:
   ```bash
   # pyproject.toml
   version = "0.2.0"

   # package.json
   "version": "0.2.0"
   ```

2. **CHANGELOG 작성**:
   ```markdown
   ## [0.2.0] - 2025-11-26
   ### Added
   - 새로운 기능

   ### Fixed
   - 버그 수정
   ```

3. **빌드**:
   ```bash
   ./build.sh
   ```

4. **테스트**:
   ```bash
   pip install dist/hdsp_agent-0.2.0-py3-none-any.whl
   jupyter lab
   # 기능 테스트
   ```

5. **Git 태그**:
   ```bash
   git tag v0.2.0
   git push origin v0.2.0
   ```

6. **배포**:
   - PyPI 업로드 (선택):
     ```bash
     poetry publish
     ```
   - Docker 이미지 빌드 및 푸시

---

## 12. 참고 자료

### 12.1 공식 문서

- **JupyterLab Extensions**: https://jupyterlab.readthedocs.io/en/stable/extension/extension_dev.html
- **Jupyter Server**: https://jupyter-server.readthedocs.io/
- **Google Gemini API**: https://ai.google.dev/docs
- **OpenAI API**: https://platform.openai.com/docs
- **vLLM**: https://docs.vllm.ai/

### 12.2 관련 기술

- **TypeScript**: https://www.typescriptlang.org/
- **React**: https://react.dev/
- **Material-UI**: https://mui.com/
- **Poetry**: https://python-poetry.org/
- **aiohttp**: https://docs.aiohttp.org/

### 12.3 Repository

- **GitHub**: https://github.com/lks21c/hdsp_agent
- **Issues**: https://github.com/lks21c/hdsp_agent/issues

---

## 13. 변경 이력

| 날짜 | 버전 | 변경 내용 |
|------|------|-----------|
| 2025-11-25 | 0.1.0 | 초기 릴리스 |
| 2025-11-25 | - | 인수인계 문서 작성 |

---

## 14. 연락처

문의사항이 있으시면 다음으로 연락 주세요:

- **Email**: your.email@example.com
- **GitHub Issues**: https://github.com/lks21c/hdsp_agent/issues

---

**문서 끝**
